...
SRC_URI = "git://gitlab.com/xen-project/imagebuilder.git;protocol=https;branch=master \
    ...
    file://mxc_remove_fit_image_size_check.patch \
    file://mxc_create_its_file.patch \
"
...
IMAGEBUILDER_COMMON_SCRIPT_FILE = "common"
FILES:${PN} = "${bindir}/${IMAGEBUILDER_SCRIPT_FILE}"

do_install() {
    install -d ${D}/${bindir}
    install -m 0755 ${S}/scripts/${IMAGEBUILDER_SCRIPT_FILE} ${D}${bindir}/
    install -m 0755 ${S}/scripts/${IMAGEBUILDER_COMMON_SCRIPT_FILE} ${D}${bindir}/
}
